...
│   ├── requestBodies.yaml           # リクエストボディ定義（スキーマへの参照）
│   ├── responses.yaml               # エラーレスポンス定義
...
| `components/requestBodies.yaml` | リクエストボディのラッパー定義 | 低 |
| `components/responses.yaml` | 共通エラーレスポンス（401, 403, 404, 409, 422, 500） | 低 |
...
- `SettingsNotifications` → レスポンスとリクエストで同一スキーマを共有
- `ErrorResponse` → 全エラーレスポンスの統一フォーマット
...
- リクエストスキーマのサフィックス: `*Request`
- レスポンススキーマのサフィックス: `*Response`
...
| エラーレスポンス | `components/responses.yaml` | 401/403/404/409/422/500 |
...
| パターン | 例 | 説明 |
|---|---|---|
| `{Domain}Response` | `DashboardResponse` | API レスポンス全体 |
| `{Domain}{Sub}Request` | `AttendanceClockInRequest` | リクエストボディ |
| `{Domain}{Sub}` | `DashboardStats` | レスポンス内の部品DTO |
| `Update{Domain}Request` | `UpdateSettingsRequest` | 更新リクエスト |
...
| パターン | 用途 |
|---|---|
| `{Domain}Response` | 単一リソースのレスポンス |
| `{Domain}MembersResponse` | 一覧レスポンス |
| `ErrorResponse` | 共通エラー |
...
3. 対応する Resource を作成（レスポンス構造を一致させる）
4. テストでレスポンス構造を検証
...
`SettingsNotifications` のようにリクエストとレスポンスで同一構造を持つ場合、
...
4. リクエストボディがあれば `components/requestBodies.yaml` に追加
